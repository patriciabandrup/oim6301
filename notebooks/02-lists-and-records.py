...
def _():
    x = 4//3
    # showing the integer of the division result (for example: 4/3 = 1.33 -> 1)
    type(x)
    print(x)
    return
...
@app.cell
def _():
    freight_charges = [16.75, 22.25 , 25.00 , 18.50 , 30.00]
...
def _(mo):
    mo.md(r"""
    when a score satisfies two of these tests at once, which one decides what gets printed? A. The first one that matches will show.
    """)
    return
